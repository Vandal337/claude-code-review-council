...
Two things worth being exact about, so this isn't oversold either:

- **The hook derives its own trusted reference — it does not, and must not, ask the orchestrator what the base revision is.** It resolves the repository's `origin/HEAD` (falling back to `origin/main`, then `origin/master`), and denies dispatch of any `review-council:*` specialist if governance files (`CLAUDE.md`, `CLAUDE.local.md`, `AGENTS.md`, `.claude/`, `.github/workflows/`) differ — committed or uncommitted — between that ref and the current checkout. If none of those refs can be resolved (no `origin` remote, unusual branch naming), the hook fails open and says so on stderr; `SKILL.md` step 2's behavioral check is what's left covering that repository.
- **A deliberate escape hatch exists**: setting the environment variable `REVIEW_COUNCIL_ALLOW_UNTRUSTED_CHECKOUT=1` bypasses the hook. This is intentional — a repository's own maintainers legitimately evolving `CLAUDE.md` on a long-lived feature branch shouldn't be permanently locked out of reviewing their own branch. The bypass requires a human to set an environment variable outside the conversation; reviewed content (a prompt injection payload) has no path to setting it.
